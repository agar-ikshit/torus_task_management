...
const TaskList = () => {
  ...
  const handleGenerateReport = async (taskId) => {
    try {
      const response = await axios.get(`https://torus-backend-xr0w.onrender.com/api/reports/summary`, {
        params: {
          ids: taskId,
  ...
  const handleDelete = async (taskId) => {
    try {
      await axios.delete(`https://torus-backend-xr0w.onrender.com/api/tasks${taskId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  const handleUpdateTask = async (updatedTask) => {
    try {
      await axios.put(`https://torus-backend-xr0w.onrender.com/api/tasks${updatedTask._id}`, updatedTask, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
      });
